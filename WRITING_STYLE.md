...
This document defines the writing conventions for NexaSign documentation.
...
1. **Users** - People using the NexaSign web application to send and sign documents
2. **Developers** - Building integrations with the API or SDKs
3. **Self-hosters** - Running their own NexaSign instance
...
```typescript
// Basic usage
const document = await nexasign.documents.create({
  title: "Contract",
  file: pdfBuffer,
});
