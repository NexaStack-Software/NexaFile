...
if [ ! -z "$DOCKER_REPOSITORY" ]; then
    ...
    docker tag "nexasign-base" "$DOCKER_REPOSITORY:latest"
    docker tag "nexasign-base" "$DOCKER_REPOSITORY:$GIT_SHA"

    # Add version tag if available
    if [ ! -z "$APP_VERSION" ] && [ "$APP_VERSION" != "undefined" ]; then
        docker tag "nexasign-base" "$DOCKER_REPOSITORY:$APP_VERSION"
    fi
else
