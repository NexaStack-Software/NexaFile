...
4. Set up your signing certificate. You have three options:
   ...
   **Option A: Generate Certificate Inside Container (Recommended)**
   ...
   docker exec -e CERT_PASS="$CERT_PASS" -it nexasign-production-nexasign-1 bash -c "
   ...
   # Restart container
   docker-compose restart nexasign
   ```
   ...
   **Option B: Use Existing Certificate**
   ...
   ```yaml
   volumes:
     - /path/to/your/cert.p12:/opt/nexasign/cert.p12:ro
   ```
...
3. **Verify Docker mount:**
   ```bash
   docker exec -it <container_name> ls -la /opt/nexasign/cert.p12
   ```
...
```bash
# For Docker Compose
docker-compose logs -f nexasign
...
| `NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH`                         | The path to the key file, default `/opt/nexasign/cert.p12`.                                        |
