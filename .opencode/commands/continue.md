...
npm run typecheck -w @nexasign/remix         # Check for type errors
...
2. **Typecheck** - Run `npm run typecheck -w @nexasign/remix` to verify types
3. **Lint** - Run `npm run lint:fix` to fix linting issues
4. **Test** - If non-trivial, run E2E tests: `npm run test:dev -w @nexasign/app-tests`
...
```bash
# Type checking
npm run typecheck -w @nexasign/remix

# Linting
npm run lint:fix

# E2E Tests (only for non-trivial work)
npm run test:dev -w @nexasign/app-tests              # Run E2E tests in dev mode
npm run test-ui:dev -w @nexasign/app-tests            # Run E2E tests with UI
