...
| `@nexasign/*` (internal)                       | varies     | No (use `workspace:*`) |
| `@aws-sdk/*`                                    | 2          | Yes                    |
| `hono`                                          | 2          | Yes                    |
...
All `@nexasign/*` internal package references currently use `"*"`. Convert to pnpm's `workspace:*` protocol:

```json
{
  "dependencies": {
    "@nexasign/lib": "workspace:*",
    "@nexasign/prisma": "workspace:*"
  }
}
```
...
5. **Turbo + pnpm compatibility:** Turbo has first-class pnpm support, but `turbo prune` output format may differ slightly.
   - **Mitigation:** Test `turbo prune --scope=@nexasign/remix --docker` and verify output structure before updating Dockerfile.
