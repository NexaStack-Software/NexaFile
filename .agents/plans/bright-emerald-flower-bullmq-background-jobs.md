...
1. **Constructor / `getInstance()`**
   ...
   - Create a single `Queue` instance for dispatching jobs, using `NEXT_PRIVATE_REDIS_PREFIX` as the BullMQ `prefix` option (defaults to `nexasign` if unset). This namespaces all Redis keys so multiple environments (worktrees, branches, developers) sharing the same Redis instance don't collide.
...
NEXT_PRIVATE_REDIS_PREFIX="nexasign"  # Namespace for Redis keys (useful when sharing a Redis instance)
```
