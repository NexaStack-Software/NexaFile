...
You are creating proper markdown documentation for a feature or guide in the NexaSign documentation site.
...
| ------------------- | ------------------------------------------------ | -------------------------------------------------- |
| **User Guide**      | `apps/docs/content/docs/users/<section>/`        | UI workflows for using the NexaSign web app       |
...
description: Upload documents to NexaSign to prepare them for signing. Covers supported formats, file size limits, and upload methods.
---
```
...
<Callout type="warn">
  ...
  [OpenAPI Reference](https://openapi.nexasign.com).
</Callout>
...
```typescript
const response = await fetch('https://app.nexasign.com/api/v2/document', {
  method: 'POST',
  headers: {
...
description: Complete reference for all environment variables used to configure NexaSign.
---
