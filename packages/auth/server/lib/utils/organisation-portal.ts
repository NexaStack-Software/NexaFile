...
import { symmetricDecrypt } from '@nexasign/lib/universal/crypto';
import { formatOrganisationCallbackUrl } from '@nexasign/lib/utils/organisation-authentication-portal';
...
export const getOrganisationAuthenticationPortalOptions = async (
  ...
  const clientSecret = Buffer.from(
    symmetricDecrypt({ key: NEXASIGN_ENCRYPTION_KEY, data: encryptedClientSecret }),
  ).toString('utf-8');
