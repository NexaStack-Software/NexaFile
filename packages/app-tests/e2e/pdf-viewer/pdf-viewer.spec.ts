...
import { createEmbeddingPresignToken } from '@nexasign/lib/server-only/embedding-presign/create-embedding-presign-token';
import { createApiToken } from '@nexasign/lib/server-only/public-api/create-api-token';
import { prefixedId } from '@nexasign/lib/universal/id';
import {
  mapSecondaryIdToDocumentId,
  mapSecondaryIdToTemplateId,
} from '@nexasign/lib/utils/envelope';
import { formatDirectTemplatePath } from '@nexasign/lib/utils/templates';
import { prisma } from '@nexasign/prisma';
import {
  seedBlankDocument,
  seedCompletedDocument,
  seedPendingDocumentWithFullFields,
} from '@nexasign/prisma/seed/documents';
import { seedBlankTemplate, seedDirectTemplate } from '@nexasign/prisma/seed/templates';
  ...
  test.describe('Recipient Signing', () => {
    test('should render PDF on signing page (V1 and V2)', async ({ page }) => {
      ...
      const { recipients: recipientsV1 } = await seedPendingDocumentWithFullFields({
        owner: user,
        teamId: team.id,
        recipients: ['signer-v1@test.nexasign.com'],
      ...
      const { document: documentV2, recipients: recipientsV2 } =
        await seedPendingDocumentWithFullFields({
          owner: user,
          teamId: team.id,
          recipients: ['signer-v2@test.nexasign.com'],
          fields: [FieldType.SIGNATURE],
  ...
  test.describe('Embed Pages', () => {
    test('should render PDF on embed sign page (V1 and V2)', async ({ page }) => {
      ...
      const { recipients: recipientsV1 } = await seedPendingDocumentWithFullFields({
        owner: user,
        teamId: team.id,
        recipients: ['embed-signer-v1@test.nexasign.com'],
        fields: [FieldType.SIGNATURE],
      });
      ...
      const { document: documentV2, recipients: recipientsV2 } =
        await seedPendingDocumentWithFullFields({
          owner: user,
          teamId: team.id,
          recipients: ['embed-signer-v2@test.nexasign.com'],
          fields: [FieldType.SIGNATURE],
          updateDocumentOptions: { internalVersion: 2 },
        });
      await addSecondEnvelopeItem(documentV2.id);
    ...
    test('should render PDF on embed multisign page (V1 and V2)', async ({ page }) => {
      ...
      const { recipients: recipientsV1 } = await seedPendingDocumentWithFullFields({
        owner: user,
        teamId: team.id,
        recipients: ['multisign-v1@test.nexasign.com'],
        fields: [FieldType.SIGNATURE],
      });
      ...
      const { document: documentV2, recipients: recipientsV2 } =
        await seedPendingDocumentWithFullFields({
          owner: user,
          teamId: team.id,
          recipients: ['multisign-v2@test.nexasign.com'],
          fields: [FieldType.SIGNATURE],
          updateDocumentOptions: { internalVersion: 2 },
        });
      await addSecondEnvelopeItem(documentV2.id);
    ...
    test('should render PDF on embed authoring document create page', async ({ page }) => {
      ...
      await emailInput.fill('test-signer@nexasign.com');
