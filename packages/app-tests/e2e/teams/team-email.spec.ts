...
import { seedTeamEmailVerification } from '@nexasign/prisma/seed/teams';
...
test('[TEAMS]: send team email request', async ({ page }) => {
  ...
  await page.getByPlaceholder('eg. Legal').fill('test@test.nexasign.com');
  await page.getByPlaceholder('example@example.com').click();
  await page.getByPlaceholder('example@example.com').fill('test@test.nexasign.com');
...
test('[TEAMS]: accept team email request', async ({ page }) => {
  ...
  const teamEmailVerification = await seedTeamEmailVerification({
    email: `team-email-verification--${team.url}@test.nexasign.com`,
    teamId: team.id,
  });
