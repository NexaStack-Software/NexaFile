...
import { EnvelopeType, RecipientRole } from '@nexasign/prisma/client';
import { seedTeam } from '@nexasign/prisma/seed/teams';
...
test.describe('Default Recipients', () => {
  test('[DEFAULT_RECIPIENTS]: default recipients are added to documents created via UI', async ({
    ...
    await page.getByTestId('signer-email-input').last().fill('regular-signer@nexasign.com');
    await page
    ...
    await expect(async () => {
      ...
      const regularSigner = envelope.recipients.find(
        (r) => r.email === 'regular-signer@nexasign.com',
      );
  ...
  test.skip('[DEFAULT_RECIPIENTS]: default recipients cannot be removed from a document', async ({
    ...
    await page.getByTestId('signer-email-input').first().fill('regular-signer@nexasign.com');
    await page
      .getByPlaceholder(/Recipient/)
      .first()
      .fill('Regular Signer');
  ...
  test('[DEFAULT_RECIPIENTS]: documents created via API have default recipients', async ({
    ...
    const payload: TCreateEnvelopePayload = {
      ...
      recipients: [
        {
          email: 'api-recipient@nexasign.com',
          name: 'API Recipient',
    ...
    const apiRecipient = envelope.recipients.find((r) => r.email === 'api-recipient@nexasign.com');
  ...
  test('[DEFAULT_RECIPIENTS]: documents created from template have default recipients', async ({
    ...
    await page.getByPlaceholder('Email').fill('template-recipient@nexasign.com');
    ...
    const templateRecipient = document.recipients.find(
      (r) => r.email === 'template-recipient@nexasign.com',
    );
