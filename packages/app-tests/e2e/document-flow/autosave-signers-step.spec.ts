...
import { getEnvelopeById } from '@nexasign/lib/server-only/envelope/get-envelope-by-id';
...
import { seedBlankDocument } from '@nexasign/prisma/seed/documents';
...
const addSignerAndSave = async (page: Page) => {
  await page.getByPlaceholder('Email').fill('recipient1@nexasign.com');
...
test.describe('AutoSave Signers Step', () => {
  test('should autosave the signers addition', async ({ page }) => {
    ...
    await expect(async () => {
      ...
      expect(retrievedRecipients[0].email).toBe('recipient1@nexasign.com');
  ...
  test('should autosave the signer update', async ({ page }) => {
    ...
    await page.getByPlaceholder('Name').fill('NexaSign Manager');
    await page.getByPlaceholder('Email').fill('manager@nexasign.com');
    ...
    await expect(async () => {
      ...
      expect(retrievedRecipients.length).toBe(1);
      expect(retrievedRecipients[0].email).toBe('manager@nexasign.com');
      expect(retrievedRecipients[0].name).toBe('NexaSign Manager');
  ...
  test('should autosave the signing order change', async ({ page }) => {
    ...
    await page.getByTestId('signer-email-input').nth(1).fill('recipient2@nexasign.com');
    await page.getByLabel('Name').nth(1).fill('Recipient 2');

    await page.getByRole('button', { name: 'Add Signer' }).click();

    await page.getByTestId('signer-email-input').nth(2).fill('recipient3@nexasign.com');
    ...
    await expect(async () => {
      ...
      const firstRecipient = retrievedRecipients.find((r) => r.email === 'recipient1@nexasign.com');
      const secondRecipient = retrievedRecipients.find(
        (r) => r.email === 'recipient2@nexasign.com',
      );
      const thirdRecipient = retrievedRecipients.find((r) => r.email === 'recipient3@nexasign.com');
