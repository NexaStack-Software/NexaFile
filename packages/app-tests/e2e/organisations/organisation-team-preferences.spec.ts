...
import { DocumentVisibility } from '@nexasign/prisma/client';
import { seedTeamDocumentWithMeta } from '@nexasign/prisma/seed/documents';
...
test('[ORGANISATIONS]: manage branding preferences', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Brand Website' }).fill('https://nexasign.com');
  ...
  expect(teamSettings.brandingUrl).toEqual('https://nexasign.com');
  ...
  expect(inheritedTeamSettings.brandingUrl).toEqual('https://nexasign.com');
...
test('[ORGANISATIONS]: manage email preferences', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Reply to email' }).fill('organisation@nexasign.com');
  ...
  // Check that the team settings have inherited these values.
  expect(teamSettings.emailReplyTo).toEqual('organisation@nexasign.com');
  ...
  // Check that the team settings now inherit from organisation again.
  expect(inheritedTeamSettings.emailReplyTo).toEqual('organisation@nexasign.com');
  ...
  expect(documentMeta.emailReplyTo).toEqual('organisation@nexasign.com');
