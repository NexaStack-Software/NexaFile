...
import { seedTeam, seedTeamMember } from '@nexasign/prisma/seed/teams';
...
test('[TEMPLATE]: should create a document from a template', async ({ page }) => {
  ...
  await page.getByLabel('Redirect URL').fill('https://nexasign.com');
  ...
  expect(document.documentMeta?.redirectUrl).toEqual('https://nexasign.com');
...
test('[TEMPLATE]: should create a document from a template with custom document', async ({
  ...
  // Add a signer
  await page.getByPlaceholder('Email').fill('recipient@nexasign.com');
