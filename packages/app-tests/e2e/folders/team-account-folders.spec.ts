...
import { DocumentVisibility, FolderType, TeamMemberRole } from '@nexasign/prisma/client';
import { seedBlankDocument, seedTeamDocuments } from '@nexasign/prisma/seed/documents';
import { seedBlankFolder } from '@nexasign/prisma/seed/folders';
import { seedTeamMember } from '@nexasign/prisma/seed/teams';
...
test('[TEAMS]: can create a document inside a document folder', async ({ page }) => {
  ...
  await fileChooser.setFiles(path.join(__dirname, '../../../assets/nexasign-supporter-pledge.pdf'));

  await page.waitForTimeout(3000);

  await expectTextToBeVisible(page, 'nexasign-supporter-pledge.pdf');
...
test('[TEAMS]: can pin a document folder', async ({ page }) => {
  ...
  await expect(page.locator('svg.text-primary.h-3.w-3')).toBeVisible();
});
...
test('[TEAMS]: can unpin a document folder', async ({ page }) => {
  ...
  await expect(page.locator('svg.text-primary.h-3.w-3')).not.toBeVisible();
});
