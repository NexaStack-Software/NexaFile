...
import { DocumentStatus, EnvelopeType, FieldType, RecipientRole } from '@nexasign/prisma/client';
...
import type { TUpdateEnvelopeItemsRequest } from '@nexasign/trpc/server/envelope-router/update-envelope-items.types';
...
const distributeEnvelope = async (
  ...
  const recipientEmail = `signer-${Date.now()}@test.nexasign.com`;
