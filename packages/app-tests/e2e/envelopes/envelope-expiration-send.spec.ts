...
import { seedPendingDocument } from '@nexasign/prisma/seed/documents';
...
test('[ENVELOPE_EXPIRATION]: sending document sets expiresAt on recipients', async ({
  ...
  const createPayload: TCreateEnvelopePayload = {
    ...
    recipients: [
      {
        email: 'signer-expiry@test.nexasign.com',
        name: 'Signer Expiry',
...
test('[ENVELOPE_EXPIRATION]: sending document with custom org expiration period', async ({
  ...
  const createPayload: TCreateEnvelopePayload = {
    ...
    recipients: [
      {
        email: 'signer-custom@test.nexasign.com',
        name: 'Signer Custom',
        role: RecipientRole.SIGNER,
        fields: [
          {
...
test('[ENVELOPE_EXPIRATION]: sending document with expiration disabled', async ({ request }) => {
  ...
  const createPayload: TCreateEnvelopePayload = {
    ...
    recipients: [
      {
        email: 'signer-disabled@test.nexasign.com',
...
test('[ENVELOPE_EXPIRATION]: resending refreshes expiresAt', async ({ page }) => {
  ...
  const document = await seedPendingDocument(user, team.id, ['resend-target@test.nexasign.com']);
  ...
  await page.getByLabel('test.nexasign.com').first().click();
