...
import type { TCachedLicense } from '@nexasign/lib/types/license';
import { seedUser } from '@nexasign/prisma/seed/users';

import { apiSignin } from '../fixtures/authentication';

const LICENSE_FILE_NAME = '.nexasign-license.json';
const LICENSE_BACKUP_FILE_NAME = '.nexasign-license-backup.json';
...
const getLicenseFilePath = () => {
  // From packages/app-tests/e2e/license -> ../../../../apps/remix/.nexasign-license.json
...
test.describe.skip('License Status Banner', () => {
  ...
  test('[ADMIN]: no banner when license file is missing', async ({ page }) => {
    ...
    await expect(
      page.getByText('This is an expired license instance of NexaSign'),
    ).not.toBeVisible();
  ...
  test.skip('[ADMIN]: global banner shows when EXPIRED with unauthorized usage', async ({
    ...
    await expect(page.getByText('This is an expired license instance of NexaSign')).toBeVisible();
    ...
    await expect(
      page.getByText('Your NexaSign instance is using features that are not part of your license.'),
    ).toBeVisible();
  });
