...
import { ALIGNMENT_TEST_FIELDS } from '@nexasign/app-tests/constants/field-alignment-pdf';
import { FIELD_META_TEST_FIELDS } from '@nexasign/app-tests/constants/field-meta-pdf';
import { isBase64Image } from '@nexasign/lib/constants/signatures';
import {
  incrementDocumentId,
  incrementTemplateId,
} from '@nexasign/lib/server-only/envelope/increment-id';
...
export const seedDatabase = async () => {
  ...
  const exampleUserExists = await prisma.user.findFirst({
    where: {
      email: 'example@nexasign.com',
    },
  });

  const adminUserExists = await prisma.user.findFirst({
    where: {
      email: 'admin@nexasign.com',
    },
  });
