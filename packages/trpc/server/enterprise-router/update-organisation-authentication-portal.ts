...
import { NEXASIGN_ENCRYPTION_KEY } from '@nexasign/lib/constants/crypto';
...
import { symmetricEncrypt } from '@nexasign/lib/universal/crypto';
import { buildOrganisationWhereQuery } from '@nexasign/lib/utils/organisations';
...
export const updateOrganisationAuthenticationPortalRoute = authenticatedProcedure
  ...
  .mutation(async ({ input, ctx }) => {
    ...
    if (clientSecret) {
      const encryptionKey = NEXASIGN_ENCRYPTION_KEY;

      if (!encryptionKey) {
        throw new Error('Missing NEXASIGN_ENCRYPTION_KEY');
      }
