...
import { AppError, genericErrorCodeToTrpcErrorCodeMap } from '@nexasign/lib/errors/app-error';
import { getApiTokenByToken } from '@nexasign/lib/server-only/public-api/get-api-token-by-token';
import type { TrpcApiLog } from '@nexasign/lib/types/api-logs';
import type { ApiRequestMetadata } from '@nexasign/lib/universal/extract-request-metadata';
import { alphaid } from '@nexasign/lib/universal/id';
...
export const authenticatedMiddleware = t.middleware(async ({ ctx, next, path, meta }) => {
  ...
  // Taken from `authenticatedMiddleware` in `@nexasign/api/v1/middleware/authenticated.ts`.
