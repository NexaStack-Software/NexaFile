...
const LICENSE_KEY =
  env('NEXT_PRIVATE_NEXASIGN_LICENSE_KEY') || env('NEXT_PRIVATE_NEXASIGN_LICENSE_KEY');
const LICENSE_SERVER_URL = env('INTERNAL_OVERRIDE_LICENSE_SERVER_URL') || 'https://nexastack.co';

declare global {
  // eslint-disable-next-line no-var
  var __nexasign_license_client__: LicenseClient | undefined;
}
...
export class LicenseClient {
  ...
  public static async start(): Promise<void> {
    if (globalThis.__nexasign_license_client__) {
      return;
    }

    const instance = new LicenseClient();

    globalThis.__nexasign_license_client__ = instance;
  ...
  public static getInstance(): LicenseClient | null {
    return globalThis.__nexasign_license_client__ ?? null;
  }
