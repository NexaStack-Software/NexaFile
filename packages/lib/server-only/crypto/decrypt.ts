import { NEXASIGN_ENCRYPTION_SECONDARY_KEY } from '@nexasign/lib/constants/crypto';
import { ZEncryptedDataSchema } from '@nexasign/lib/server-only/crypto/encrypt';
...
export const decryptSecondaryData = (encryptedData: string): string | null => {
  if (!NEXASIGN_ENCRYPTION_SECONDARY_KEY) {
    throw new Error('Missing encryption key');
  }

  try {
    const decryptedBufferValue = symmetricDecrypt({
      key: NEXASIGN_ENCRYPTION_SECONDARY_KEY,
      data: encryptedData,
    });
