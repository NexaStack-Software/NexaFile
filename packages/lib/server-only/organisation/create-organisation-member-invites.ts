...
import { syncMemberCountWithStripeSeatPlan } from '@nexasign/ee/server-only/stripe/update-subscription-item-quantity';
import { mailer } from '@nexasign/email/mailer';
import { OrganisationInviteEmailTemplate } from '@nexasign/email/templates/organisation-invite';
import { NEXT_PUBLIC_WEBAPP_URL } from '@nexasign/lib/constants/app';
import { ORGANISATION_MEMBER_ROLE_PERMISSIONS_MAP } from '@nexasign/lib/constants/organisations';
...
import type { TCreateOrganisationMemberInvitesRequestSchema } from '@nexasign/trpc/server/organisation-router/create-organisation-member-invites.types';
...
export const sendOrganisationMemberInviteEmail = async ({
  ...
  await mailer.sendMail({
    to: email,
    from: senderEmail,
    subject: i18n._(msg`You have been invited to join ${organisation.name} on NexaFile`),
    html,
    text,
  });
};
