...
import type { BrandingSettings } from '@nexasign/email/providers/branding';
...
export const getEmailContext = async (
  ...
  if (options.emailType === 'INTERNAL') {
    return {
      ...emailContext,
      senderEmail: NEXASIGN_INTERNAL_EMAIL,
  ...
  const senderEmailId = match(meta?.emailId)
    ...
    .with(null, () => null) // Explicit null means to use the NexaSign email.
    .exhaustive();
