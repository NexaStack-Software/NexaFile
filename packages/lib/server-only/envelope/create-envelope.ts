...
import type { PlaceholderInfo } from '@nexasign/lib/server-only/pdf/auto-place-fields';
import { convertPlaceholdersToFieldInputs } from '@nexasign/lib/server-only/pdf/auto-place-fields';
import { findRecipientByPlaceholder } from '@nexasign/lib/server-only/pdf/helpers';
import { normalizePdf as makeNormalizedPdf } from '@nexasign/lib/server-only/pdf/normalize-pdf';
...
import { nanoid, prefixedId } from '@nexasign/lib/universal/id';
    ...
    if (itemsWithPlaceholders.length > 0) {
      ...
      if (shouldCreatePlaceholderRecipients) {
        ...
        const placeholderRecipients = Array.from(
          ...
          ([recipientIndex, name]) => ({
            envelopeId: envelope.id,
            email: `recipient.${recipientIndex}@nexasign.com`,
            name,
