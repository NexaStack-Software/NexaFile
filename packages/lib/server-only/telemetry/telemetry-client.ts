...
const HAS_LICENSE_KEY = !!(
  process.env.NEXT_PRIVATE_NEXASIGN_LICENSE_KEY || process.env.NEXT_PRIVATE_NEXASIGN_LICENSE_KEY
);
...
const TELEMETRY_DISABLED =
  !!(process.env.NEXASIGN_DISABLE_TELEMETRY || process.env.NEXASIGN_DISABLE_TELEMETRY) ||
  HAS_LICENSE_KEY;

const NODE_ID_FILENAME = '.nexasign-node-id';
...
export class TelemetryClient {
   ...
   *
   * If telemetry is disabled via `NEXASIGN_DISABLE_TELEMETRY=true` or credentials are not
  ...
  public static async start(): Promise<void> {
    if (TELEMETRY_DISABLED) {
      if (!HAS_LICENSE_KEY) {
        console.log(
          '[Telemetry] Telemetry is disabled. To enable, remove the NEXASIGN_DISABLE_TELEMETRY environment variable.',
        );
      }
  ...
  private async initialize(): Promise<void> {
    ...
    console.log(
      '[Telemetry] To disable telemetry, set NEXASIGN_DISABLE_TELEMETRY=true in your environment variables.',
    );
    console.log(
      '[Telemetry] Learn more: https://nexasign.nexastack.co/docs/self-hosting/configuration/telemetry',
    );
