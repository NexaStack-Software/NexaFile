...
import { useCopyShareLink } from '@nexasign/lib/client-only/hooks/use-copy-share-link';
...
import { generateTwitterIntent } from '@nexasign/lib/universal/generate-twitter-intent';
  ...
  const onTweetClick = async () => {
    ...
    window.open(
      generateTwitterIntent(
        `Ich habe gerade ein Dokument mit @nexasign ${token ? 'unterzeichnet' : 'versendet'}. Hier anschauen!`,
      ...
      <DialogContent position="end">
        ...
        <div className="flex w-full flex-col">
          <div className="rounded-md border p-4">
            Ich habe gerade ein Dokument mit{' '}
            <span className="font-medium text-blue-400">@nexasign</span>{' '}
            {token ? 'unterzeichnet' : 'versendet'}. Hier anschauen!
