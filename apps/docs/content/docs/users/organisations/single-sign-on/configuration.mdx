...
NexaFile supports any identity provider that implements the OpenID Connect (OIDC) protocol. Common providers include:
...
- **Redirect URI**: The value from the NexaFile SSO settings page
...
Back in NexaFile, enter the following:
...
NexaFile verifies their identity with your provider.
...
- Cannot set a NexaFile password (they must use SSO)
...
<Callout type="info">
  SSO-only enforcement is not currently available. Users who have an existing NexaFile account with a password can still sign in with their original method in addition to SSO.
</Callout>
...
<Accordions type="multiple">
  ...
  - Copy the redirect URI exactly from NexaFile's SSO settings
  ...
  <Accordion title="User already exists">
    If a user already has a NexaFile account with the same email, their existing account will be linked to your team and they can sign in through your SSO portal going forward.
  </Accordion>
</Accordions>
