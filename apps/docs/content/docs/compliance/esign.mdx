...
## How NexaFile Supports Compliance

NexaFile provides features that support compliance with e-signature laws across jurisdictions:
...
NexaFile maintains an audit trail for each document, recording:
...
NexaFile supports compliance with Simple Electronic Signature (SES) requirements. The following are not currently provided:
