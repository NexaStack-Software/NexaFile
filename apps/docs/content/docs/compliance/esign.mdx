...
## How NexaSign Supports Compliance

NexaSign provides features that support compliance with e-signature laws across jurisdictions:
...
NexaSign maintains an audit trail for each document, recording:
...
NexaSign supports compliance with Simple Electronic Signature (SES) requirements. The following are not currently provided:
