...
- A NexaFile account on any plan (Free, Individual, Team, or Enterprise)

<Callout type="info">
  Free accounts include API access with a limit of 5 documents per month. [Upgrade to a paid
  plan](https://nexastack.co) for higher limits.
</Callout>
...
- Log in to your NexaFile account
- Click your avatar in the top right corner
- Select **Settings** from the dropdown menu

![User dropdown menu](/public-api-images/nexasign-user-dropdown-menu.webp)
...
![API tokens page](/public-api-images/api-tokens-page-nexasign.webp)
...
![API key display](/public-api-images/nexasign-api-key-blurred.webp)
...
```bash
curl https://nexasign.nexastack.co/api/v2/documents \
...
```typescript
const response = await fetch('https://nexasign.nexastack.co/api/v2/documents', {
  method: 'GET',
  headers: {
...
NexaFile provides official SDKs that handle authentication for you:

```typescript
import { NexaFile } from '@nexasign/sdk-typescript';

const client = new NexaFile({
...
SDKs are available for [TypeScript](https://github.com/nexasign/sdk-typescript), [Python](https://github.com/nexasign/sdk-python), and [Go](https://github.com/nexasign/sdk-go).
...
<Callout type="info">
  ...
  OpenAPI reference](https://nexasign.nexastack.co/api/v1/openapi).
</Callout>
...
NEXASIGN_API_KEY=api_xxxxxxxxxxxxxxxx
```

```typescript
const client = new NexaFile({
  apiKey: process.env.NEXASIGN_API_KEY,
});
```
