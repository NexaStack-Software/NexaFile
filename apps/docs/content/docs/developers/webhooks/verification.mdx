...
Verifying webhook requests ensures that incoming payloads originate from NexaFile and have not been tampered with. Without verification, attackers could forge requests to your endpoint and trigger unintended actions.

## How NexaFile Signs Webhooks

When you configure a webhook with a secret, NexaFile includes that secret in every webhook request via the `X-NexaSign-Secret` header. Your server should compare this header value against your stored secret to authenticate the request.
...
| --------------- | --------------------------------------- | ------------------------------------------------------------------ |
| Secret mismatch | Webhook secret changed or misconfigured | Verify the secret in your environment matches the one in NexaFile |
| Empty header    | Webhook created without a secret        | Add a secret to the webhook configuration in NexaFile             |
...
3. Update the webhook configuration in NexaFile
