...
Verifying webhook requests ensures that incoming payloads originate from NexaSign and have not been tampered with. Without verification, attackers could forge requests to your endpoint and trigger unintended actions.

## How NexaSign Signs Webhooks

When you configure a webhook with a secret, NexaSign includes that secret in every webhook request via the `X-NexaSign-Secret` header. Your server should compare this header value against your stored secret to authenticate the request.

```bash
POST /your-webhook-endpoint HTTP/1.1
Host: your-server.com
Content-Type: application/json
X-NexaSign-Secret: your_webhook_secret_here
...
| Header               | Description                                             |
| -------------------- | ------------------------------------------------------- |
| `X-NexaSign-Secret` | The secret key you configured when creating the webhook |
...
<Steps>
  <Step>
    Extract the `X-NexaSign-Secret` header from the incoming request
  </Step>
  <Step>
...
const WEBHOOK_SECRET = process.env.NEXASIGN_WEBHOOK_SECRET;
...
app.post('/webhooks/nexasign', (req, res) => {
  const receivedSecret = req.headers['x-nexasign-secret'];
...
WEBHOOK_SECRET = os.environ.get('NEXASIGN_WEBHOOK_SECRET')
...
@app.route('/webhooks/nexasign', methods=['POST'])
def handle_webhook():
    received_secret = request.headers.get('X-NexaSign-Secret', '')
...
| --------------- | --------------------------------------- | ------------------------------------------------------------------ |
| Secret mismatch | Webhook secret changed or misconfigured | Verify the secret in your environment matches the one in NexaSign |
| Empty header    | Webhook created without a secret        | Add a secret to the webhook configuration in NexaSign             |
...
3. Update the webhook configuration in NexaSign
...
app.post('/webhooks/nexasign', webhookLimiter, (req, res) => {
  // Handle webhook...
});
```
