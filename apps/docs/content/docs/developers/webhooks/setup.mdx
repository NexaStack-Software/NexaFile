...
Webhooks are HTTP callbacks triggered by specific events in NexaFile. When an event occurs (such as a document being signed), NexaFile sends an HTTP POST request to your configured URL with details about the event.
...
Before configuring a webhook in NexaFile, you need an endpoint that can receive HTTP POST requests. Here's a minimal example:
...
<Callout type="warn">
  Always respond with a `200 OK` status within 30 seconds. NexaFile will retry failed deliveries.
</Callout>

## Configuring Webhooks in NexaFile via the Dashboard
...
NexaFile provides a built-in testing feature to verify your webhook endpoint works correctly.
...
<Step>
NexaFile will attempt to deliver the same payload again
</Step>
</Steps>

## Retry Policy

When a webhook delivery fails (non-2xx response or timeout), NexaFile automatically retries with exponential backoff:
