---
title: Svelte
description: Embed NexaFile signing in your Svelte application using the official SDK.
---
...
<Tabs items={['npm', 'yarn', 'pnpm']}>
  <Tab value="npm">``` npm install @nexasign/embed-svelte ```</Tab>
  <Tab value="yarn">``` yarn add @nexasign/embed-svelte ```</Tab>
  <Tab value="pnpm">``` pnpm add @nexasign/embed-svelte ```</Tab>
</Tabs>
...
```svelte
<script lang="ts">
  import { EmbedDirectTemplate } from '@nexasign/embed-svelte';
...
```svelte
<script lang="ts">
  import { EmbedSignDocument } from '@nexasign/embed-svelte';
