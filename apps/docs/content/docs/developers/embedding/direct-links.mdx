...
When someone uses a direct link, NexaFile creates a new document from the template with that person as the signer.
...
| Account   | Signer must be logged into a NexaFile account |
...
Enterprise plans support white-label embedding with the "Powered by NexaFile" badge removed. [Contact sales](mailto:info@nexastack.co) for details.
