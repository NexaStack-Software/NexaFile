...
When someone uses a direct link, NexaSign creates a new document from the template with that person as the signer.
...
curl -X POST "https://nexasign.nexastack.co/api/v2/template/direct/create" \
...
// URL: https://nexasign.nexastack.co/d/{token}
````
</Tab>
</Tabs>
...
```
https://nexasign.nexastack.co/d/{token}
```
...
```html
<iframe
  src="https://nexasign.nexastack.co/embed/direct/abc123xyz"
  width="100%"
  height="800"
  frameborder="0"
...
function DocumentSigner({ token }: { token: string }) {
  return (
    <div className="h-[800px] w-full">
      <iframe
        src={`https://nexasign.nexastack.co/embed/direct/${token}`}
...
function redirectToSigning(token: string) {
  window.location.href = `https://nexasign.nexastack.co/d/${token}`;
}
```
...
```
https://nexasign.nexastack.co/d/abc123xyz?externalId=order-12345
```
...
function getSigningUrl(token: string, orderId: string) {
  ...
  return `https://nexasign.nexastack.co/d/${token}?${params.toString()}`;
}
...
```
https://nexasign.nexastack.co/embed/direct/abc123xyz?externalId=order-12345
```
...
| Account   | Signer must be logged into a NexaSign account |
...
<Tab value="curl">
```bash
curl -X POST "https://nexasign.nexastack.co/api/v2/template/direct/toggle" \
  -H "Authorization: api_xxxxxxxxxxxxxxxx" \
  -H "Content-Type: application/json" \
  -d '{
    "templateId": 123,
...
```bash
curl -X POST "https://nexasign.nexastack.co/api/v2/template/direct/delete" \
  -H "Authorization: api_xxxxxxxxxxxxxxxx" \
  -H "Content-Type: application/json" \
  -d '{
    "templateId": 123
...
Enterprise plans support white-label embedding with the "Powered by NexaSign" badge removed. [Contact sales](mailto:info@nexastack.co) for details.
...
function getEmbedUrl(token: string, orderId: string) {
  ...
  return `https://nexasign.nexastack.co/embed/direct/${token}?${params}`;
}
