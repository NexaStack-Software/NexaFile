...
NexaSign offers two types of embedding:
...
The [Platform Plan](https://nexastack.co) adds enhanced customization:

- Custom CSS and styling variables
- Dark mode controls
- Removal of NexaSign branding
...
```jsx
import { EmbedDirectTemplate } from '@nexasign/embed-react';
...
```jsx
import { EmbedSignDocument } from '@nexasign/embed-react';
...
<Steps>
<Step>
Navigate to your team's templates in NexaSign
...
For example, `https://nexasign.nexastack.co/d/-WoSwWVT-fYOERS2MI37k` has the token `-WoSwWVT-fYOERS2MI37k`.
...
A [Web Components](https://developer.mozilla.org/en-US/docs/Web/API/Web_components) SDK (`@nexasign/embed-webcomponent`) is also available for use outside of JavaScript frameworks. It works in any environment that supports custom elements.
...
| `host`                | `string`   | NexaSign instance URL. Defaults to `https://nexasign.nexastack.co`. |
...
| `language`            | `string`   | Set the UI language. See [Supported Languages](https://github.com/NexaStack-Software/NexaSign/tree/main/packages/lib/constants/locales.ts). |
