...
Register your endpoint in the NexaSign dashboard:
...
class NexaSignApiError extends Error {
  code?: string;
  statusCode: number;
  details?: Record<string, unknown>;
...
try {
  ...
  if (error instanceof NexaSignApiError) {
