...
app.post('/webhooks/nexasign', (req, res) => {
  ...
  const secret = req.headers['x-nexasign-secret'] as string;
...
Register your endpoint in the NexaFile dashboard:
...
<Steps>
  ...
  <Step>
    Enter your webhook URL (e.g. <code>https://your-app.com/webhooks/nexasign</code>)
  </Step>
  <Step>
  ...
  <Step>
    Enter your webhook secret (the same value stored in <code>NEXASIGN_WEBHOOK_SECRET</code>)
  </Step>
  <Step>
    Click **Create Webhook**
  </Step>
...
class NexaFileApiError extends Error {
  code?: string;
  statusCode: number;
  details?: Record<string, unknown>;
...
try {
  ...
  if (error instanceof NexaFileApiError) {
...
- [API Reference](https://nexasign.nexastack.co/api/v2/openapi.json/) - Full endpoint documentation
