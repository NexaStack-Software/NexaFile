---
title: Quick Start
description: Get NexaSign running locally in minutes with Docker Compose.
---
...
docker compose -f docker/development/compose.yml exec database \
  psql -U nexasign -d nexasign -c "UPDATE \"User\" SET roles = '{ADMIN}' WHERE email = 'your@email.com';"
```
...
| ----------- | -------------------------------- | ----- |
| `nexasign` | Main application                 | 3000  |
...
<Accordions type="multiple">
  ...
  <Accordion title="Container fails to start">
    Check the logs: `docker compose -f docker/development/compose.yml logs nexasign`. Common
