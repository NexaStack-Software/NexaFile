...
description: Practical advice for running NexaFile in production.
---
...
<Callout type="error">
  This is the single most common issue when self-hosting NexaFile. Without a signing certificate,
  the application starts normally but document signing fails.
</Callout>

NexaFile does **not** bundle a [signing certificate](/docs/self-hosting/configuration/signing-certificate) in the Docker image. You must provide one yourself using one of two methods:
...
The default certificate path inside the container is `/opt/nexasign/cert.p12`. You can change this with `NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH`.

### Base64 Environment Variable

For platforms that don't support volume mounts, encode the certificate and pass it as an environment variable:
...
After starting NexaFile, check the certificate status endpoint:
...
Always set a password when creating your `.p12` certificate. When using OpenSSL, set the password during the export step and provide it to NexaFile via `NEXT_PRIVATE_SIGNING_PASSPHRASE`.

---

## Use the Health Check Endpoints

NexaFile exposes two endpoints for monitoring:
...
docker pull NexaStack-Software/NexaSign:1.8.0

# Risky — may pull breaking changes
docker pull NexaStack-Software/NexaSign:latest
```

Check the [GitHub releases](https://github.com/NexaStack-Software/NexaSign/releases) page for available versions.
...
Background jobs work by NexaFile sending HTTP requests to itself. If your reverse proxy or network setup causes issues with the app reaching its own public URL, set the internal URL:
...
NEXT_PRIVATE_UPLOAD_BUCKET=nexasign-documents
...
NexaFile uses a PostgreSQL-based job queue by default (`local` provider). No Redis or external message broker is required for basic deployments.
...
The NexaFile container runs as a non-root user (UID 1001). If you mount files into the container (certificates, configuration), ensure they're readable:
