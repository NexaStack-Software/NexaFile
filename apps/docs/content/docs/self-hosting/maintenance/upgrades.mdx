---
title: Upgrades
description: Upgrade your self-hosted NexaFile installation to get the latest features, security patches, and bug fixes.
---
...
NexaFile follows [Semantic Versioning](https://semver.org/):
...
<Callout type="info">
  Review the [release notes](https://github.com/NexaStack-Software/NexaSign/releases) before upgrading to
...
Check your running NexaFile version:

<Tabs items={['Docker', 'Docker Compose', 'Kubernetes']}>
<Tab value="Docker">

```bash
docker inspect nexasign --format='{{.Config.Image}}'
```
...
```bash
kubectl get deployment nexasign -n nexasign -o jsonpath='{.spec.template.spec.containers[0].image}'
```
...
```bash
curl -s https://api.github.com/repos/NexaStack-Software/NexaSign/releases/latest | grep tag_name
```

Or visit the [releases page](https://github.com/NexaStack-Software/NexaSign/releases).
...
```bash
curl -s https://hub.docker.com/v2/repositories/NexaStack-Software/NexaSign/tags | jq -r '.results[].name'
```
...
```bash
curl -s https://api.github.com/orgs/nexasign/packages/container/nexasign/versions | jq -r '.[].metadata.container.tags[]'
```
...
```bash
docker compose exec database pg_dump -U nexasign nexasign > backup-$(date +%Y%m%d-%H%M%S).sql
```
...
```bash
kubectl exec -n nexasign deploy/postgres -- pg_dump -U nexasign nexasign > backup-$(date +%Y%m%d-%H%M%S).sql
```
...
```bash
pg_dump "postgresql://user:password@host:5432/nexasign" > backup-$(date +%Y%m%d-%H%M%S).sql
```
...
```bash
kubectl get secret nexasign-secrets -n nexasign -o yaml > secrets-backup.yaml
kubectl get configmap nexasign-config -n nexasign -o yaml > configmap-backup.yaml
```
...
```bash
docker pull NexaStack-Software/NexaSign:1.6.0
```
...
docker run -d \
  ...
  -p 3000:3000 \
  --env-file .env \
  -v /path/to/cert.p12:/opt/nexasign/cert.p12:ro \
  NexaStack-Software/NexaSign:1.6.0
```
...
services:
  nexasign:
    image: NexaStack-Software/NexaSign:1.6.0
```
...
services:
  nexasign:
    image: NexaStack-Software/NexaSign:${NEXASIGN_VERSION:-latest}
```
...
```bash
kubectl set image deployment/nexasign \
  nexasign=NexaStack-Software/NexaSign:1.6.0 \
  -n nexasign
```
...
```bash
kubectl rollout status deployment/nexasign -n nexasign
```

Watch pods transition:

```bash
kubectl get pods -n nexasign -w
```
...
```bash
kubectl get pods -n nexasign -o wide
```

Test the health endpoint:

```bash
kubectl port-forward svc/nexasign 3000:80 -n nexasign &
curl http://localhost:3000/api/health
```
...
NexaFile runs database migrations automatically when the container starts. No manual intervention is required.
...
```bash
docker logs nexasign 2>&1 | grep -i migration
```
...
```bash
kubectl logs -l app.kubernetes.io/name=nexasign -n nexasign | grep -i migration
```
...
# Run migrations only
docker run --rm \
  -e NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@host:5432/nexasign" \
  NexaStack-Software/NexaSign:1.6.0 \
...
Read the [release notes](https://github.com/NexaStack-Software/NexaSign/releases) carefully.
...
curl -O https://raw.githubusercontent.com/NexaStack-Software/NexaSign/main/.env.example
...
docker run -d \
  ...
  --env-file .env \
  -v /path/to/cert.p12:/opt/nexasign/cert.p12:ro \
  NexaStack-Software/NexaSign:1.5.0
```
...
kubectl rollout history deployment/nexasign -n nexasign

# Rollback to previous version
kubectl rollout undo deployment/nexasign -n nexasign

# Or rollback to a specific revision
kubectl rollout undo deployment/nexasign -n nexasign --to-revision=2
```
...
```bash
docker compose exec -T database psql -U nexasign nexasign < backup-20240115-120000.sql
```
...
```bash
psql "postgresql://user:password@host:5432/nexasign" < backup-20240115-120000.sql
```
...
<Accordions type="multiple">
  <Accordion title="Container fails to start after upgrade">
    **Check logs:** `docker logs nexasign`
  ...
  <Accordion title="Migration errors">
    ...
    **Test connectivity:**
    ...
    ```bash
    docker run --rm postgres:15 psql "postgresql://user:password@host:5432/nexasign" -c "SELECT 1"
    ```
  ...
  <Accordion title="Application errors after upgrade">
    **If the application starts but behaves unexpectedly:**
    ...
    ```bash
    docker logs nexasign 2>&1 | grep -i -E "(warn|error)"
    ```
  </Accordion>
