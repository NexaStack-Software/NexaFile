...
<Callout type="info">
  Review the [release notes](https://github.com/NexaStack-Software/NexaFile/releases) before upgrading to
...
```bash
curl -s https://api.github.com/repos/NexaStack-Software/NexaFile/releases/latest | grep tag_name
```

Or visit the [releases page](https://github.com/NexaStack-Software/NexaFile/releases).
...
```bash
curl -s https://hub.docker.com/v2/repositories/nexastack/nexafile/tags | jq -r '.results[].name'
```
...
```bash
curl -s https://api.github.com/orgs/NexaStack-Software/packages/container/nexafile/versions | jq -r '.[].metadata.container.tags[]'
```
...
```bash
docker pull nexastack/nexafile:1.6.0
```
...
services:
  nexasign:
    image: nexastack/nexafile:1.6.0
```
...
services:
  nexasign:
    image: nexastack/nexafile:${NEXASIGN_VERSION:-latest}
```
...
kubectl set image deployment/nexasign \
  nexasign=nexastack/nexafile:1.6.0 \
  -n nexasign
```
...
Read the [release notes](https://github.com/NexaStack-Software/NexaFile/releases) carefully.
...
curl -O https://raw.githubusercontent.com/NexaStack-Software/NexaFile/main/.env.example
