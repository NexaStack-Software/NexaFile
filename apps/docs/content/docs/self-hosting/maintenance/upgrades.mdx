...
<Callout type="info">
  Review the [release notes](https://github.com/nexastack/nexasign/releases) before upgrading to
...
```bash
curl -s https://api.github.com/repos/nexastack/nexasign/releases/latest | grep tag_name
```

Or visit the [releases page](https://github.com/nexastack/nexasign/releases).
...
```bash
curl -s https://hub.docker.com/v2/repositories/nexastack/nexasign/tags | jq -r '.results[].name'
```
...
```bash
docker pull nexastack/nexasign:1.6.0
```
...
services:
  nexasign:
    image: nexastack/nexasign:1.6.0
```
...
services:
  nexasign:
    image: nexastack/nexasign:${NEXASIGN_VERSION:-latest}
```
...
kubectl set image deployment/nexasign \
  nexasign=nexastack/nexasign:1.6.0 \
  -n nexasign
```
...
Read the [release notes](https://github.com/nexastack/nexasign/releases) carefully.
...
curl -O https://raw.githubusercontent.com/nexastack/nexasign/main/.env.example
