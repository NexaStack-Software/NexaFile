...
description: Diagnose and resolve common issues with your self-hosted NexaFile instance, organized by component.
---
...
```bash
kubectl logs -l app.kubernetes.io/name=nexasign -n nexasign -f
```
...
```bash
# Docker - find errors
docker logs nexasign 2>&1 | grep -i error

# Docker - find warnings
docker logs nexasign 2>&1 | grep -i warn

# Show last 100 lines
docker logs --tail 100 nexasign
```

### Health Checks

NexaFile exposes a health endpoint to verify the application is running:
...
```bash
docker exec nexasign printenv | grep NEXT
```
...
```bash
docker compose exec nexasign printenv | grep NEXT
```
...
```bash
docker inspect nexasign --format='{{.State.ExitCode}}'
```
...
```bash
# Docker
docker stats nexasign

# Kubernetes
kubectl top pod -n nexasign
```
...
<Steps>
  ...
  <Step>
    Change NexaFile's port mapping: `-p 3001:3000`
  </Step>
</Steps>
...
```
Error: password authentication failed for user "nexasign"
```
...
```sql
ALTER USER nexasign WITH PASSWORD 'newpassword';
```
...
```
Error: database "nexasign" does not exist
```
...
```
Error: too many connections for role "nexasign"
```
...
```
postgresql://user:password@host:5432/nexasign?connection_limit=5
```
...
```bash
docker exec nexasign printenv | grep SMTP
```
...
```bash
docker exec nexasign nc -zv smtp.example.com 587
```
...
```bash
docker exec nexasign printenv | grep SIGNING
```
...
```bash
docker exec nexasign ls -la /opt/nexasign/cert.p12
```
...
```
Error: EACCES: permission denied, open '/opt/nexasign/cert.p12'
```
...
```
  ...
  <Step>
    Update the certificate file and restart NexaFile
  </Step>
</Steps>
...
```bash
docker exec nexasign cat /etc/resolv.conf
```
...
- **[GitHub Discussions](https://github.com/NexaStack-Software/NexaSign/discussions)**
- **[Discord](https://github.com/NexaStack-Software/NexaSign/discussions)**: Join the NexaFile community Discord
- **[GitHub Issues](https://github.com/NexaStack-Software/NexaSign/issues)**: For confirmed bugs, open an issue
...
For security vulnerabilities, do not open a public issue. Contact security@nexastack.co directly.
...
<Accordions type="multiple">
  <Accordion title="Check application logs for errors">
    Run `docker logs nexasign` or `docker compose logs nexasign` (or `kubectl logs` for
