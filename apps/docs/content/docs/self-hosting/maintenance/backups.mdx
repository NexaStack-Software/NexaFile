---
title: Backups
description: Backup strategies for NexaFile data.
---
...
NexaFile stores data in three locations:
...
```bash
pg_dump -h localhost -U nexasign -d nexasign -F c -f nexasign_backup.dump
```
...
```bash
docker compose exec database pg_dump -U nexasign -F c nexasign > nexasign_backup.dump
```
...
```bash
pg_dump -h localhost -U nexasign -d nexasign -F c \
  -f "nexasign_$(date +%Y%m%d_%H%M%S).dump"
```
...
```bash
pg_dump -h localhost -U nexasign -d nexasign -F p -f nexasign_backup.sql
```
...
```bash
pg_dump -h localhost -U nexasign -d nexasign | gzip > nexasign_backup.sql.gz
```
...
```bash
cp /opt/nexasign/cert.p12 /backup/nexasign/cert.p12
```
...
```bash
echo "$NEXT_PRIVATE_SIGNING_LOCAL_FILE_CONTENTS" | base64 -d > /backup/nexasign/cert.p12
```
...
```bash
aws s3 sync s3://your-nexasign-bucket s3://your-backup-bucket
# or
aws s3 sync s3://your-nexasign-bucket /backup/nexasign/documents/
```
...
```bash
aws s3 sync s3://your-bucket /backup/nexasign/documents/ \
...
Create a script to back up all NexaFile components:
...
S3_BUCKET="your-nexasign-bucket"

# Create backup directory
mkdir -p "$BACKUP_DIR/$TIMESTAMP"

echo "Starting NexaFile backup: $TIMESTAMP"
...
if [ -f "/opt/nexasign/cert.p12" ]; then
  echo "Backing up certificate..."
  cp /opt/nexasign/cert.p12 "$BACKUP_DIR/$TIMESTAMP/cert.p12"
fi
...
tar -czf "$BACKUP_DIR/nexasign_$TIMESTAMP.tar.gz" -C "$BACKUP_DIR" "$TIMESTAMP"
rm -rf "$BACKUP_DIR/$TIMESTAMP"

# Calculate checksum
sha256sum "$BACKUP_DIR/nexasign_$TIMESTAMP.tar.gz" > "$BACKUP_DIR/nexasign_$TIMESTAMP.sha256"

# Remove old backups
echo "Removing backups older than $RETENTION_DAYS days..."
find "$BACKUP_DIR" -name "nexasign_*.tar.gz" -mtime +$RETENTION_DAYS -delete
find "$BACKUP_DIR" -name "nexasign_*.sha256" -mtime +$RETENTION_DAYS -delete

echo "Backup completed: nexasign_$TIMESTAMP.tar.gz"
```
...
```bash
#!/bin/bash
# nexasign-backup-docker.sh

set -e

BACKUP_DIR="/backup/nexasign"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
COMPOSE_PROJECT="nexasign-production"
...
docker compose -p "$COMPOSE_PROJECT" exec -T database \
  pg_dump -U nexasign -F c nexasign > "$BACKUP_DIR/database_$TIMESTAMP.dump"

# Certificate backup (copy from container if needed)
docker compose -p "$COMPOSE_PROJECT" cp \
  nexasign:/opt/nexasign/cert.p12 "$BACKUP_DIR/cert_$TIMESTAMP.p12" 2>/dev/null || true
...
```bash
chmod +x /usr/local/bin/nexasign-backup.sh
```
...
0 2 * * * /usr/local/bin/nexasign-backup.sh >> /var/log/nexasign-backup.log 2>&1

# Hourly backup (for high-activity instances)
0 * * * * /usr/local/bin/nexasign-backup-hourly.sh >> /var/log/nexasign-backup.log 2>&1
```

</Tab>
<Tab value="Systemd timer">

Create `/etc/systemd/system/nexasign-backup.service`:

```ini
[Unit]
Description=NexaFile Backup

[Service]
Type=oneshot
ExecStart=/usr/local/bin/nexasign-backup.sh
User=root
```

Create `/etc/systemd/system/nexasign-backup.timer`:

```ini
[Unit]
Description=Daily NexaFile Backup
...
systemctl enable --now nexasign-backup.timer
```
...
```bash
pg_restore -h localhost -U nexasign -d nexasign -c nexasign_backup.dump
```

Docker Compose:

```bash
docker compose exec -T database pg_restore -U nexasign -d nexasign -c < nexasign_backup.dump
```
...
```bash
psql -h localhost -U nexasign -d nexasign < nexasign_backup.sql
```

For compressed backups:

```bash
gunzip -c nexasign_backup.sql.gz | psql -h localhost -U nexasign -d nexasign
```
...
```bash
cp /backup/nexasign/cert.p12 /opt/nexasign/cert.p12
chmod 644 /opt/nexasign/cert.p12
chown 1001:1001 /opt/nexasign/cert.p12
```
...
```bash
aws s3 sync /backup/nexasign/documents/ s3://your-nexasign-bucket
```
...
```bash
docker compose exec -T database pg_restore -U nexasign -d nexasign -c < backup.dump
```
...
```bash
docker compose cp cert.p12 nexasign:/opt/nexasign/cert.p12
```
...
```bash
# Check checksum
sha256sum -c nexasign_backup.sha256

# Verify pg_dump format
pg_restore --list nexasign_backup.dump > /dev/null && echo "Backup is valid"
```
...
```bash
aws s3 cp nexasign_backup.tar.gz s3://your-backup-bucket/nexasign/
```
...
```bash
# Encrypt
gpg --symmetric --cipher-algo AES256 nexasign_backup.tar.gz

# Decrypt
gpg --decrypt nexasign_backup.tar.gz.gpg > nexasign_backup.tar.gz
```
