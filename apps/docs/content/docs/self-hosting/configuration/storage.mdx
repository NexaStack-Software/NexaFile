...
S3 storage is recommended for production deployments. NexaSign supports AWS S3 and any S3-compatible storage service.
...
```bash
aws s3 mb s3://your-nexasign-bucket --region us-east-1
```
...
[
  {
    ...
    "AllowedOrigins": ["https://your-nexasign-domain.com"],
    "ExposeHeaders": ["ETag"]
  }
]
```
...
```bash
aws s3api put-bucket-cors --bucket your-nexasign-bucket --cors-configuration file://cors.json
```
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:s3:::your-nexasign-bucket/*"
    }
  ]
}
```
...
Using Docker Compose with NexaSign:
...
```bash
NEXT_PUBLIC_UPLOAD_TRANSPORT=s3
NEXT_PRIVATE_UPLOAD_BUCKET=nexasign
...
NexaSign works with any S3-compatible storage service. Configure the endpoint and enable path-style URLs if required.
...
- An S3 bucket configured for NexaSign
...
NexaSign does not provide automatic migration between storage backends. Each document's storage location is recorded in the database.
...
During migration, NexaSign can read from both backends. The `DocumentData.type` field indicates where each document is stored.
