...
S3 storage is recommended for production deployments. NexaFile supports AWS S3 and any S3-compatible storage service.
...
Using Docker Compose with NexaFile:
...
NexaFile works with any S3-compatible storage service. Configure the endpoint and enable path-style URLs if required.
...
- An S3 bucket configured for NexaFile
...
NexaFile does not provide automatic migration between storage backends. Each document's storage location is recorded in the database.
...
During migration, NexaFile can read from both backends. The `DocumentData.type` field indicates where each document is stored.
