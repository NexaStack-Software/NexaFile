...
description: Configure PostgreSQL connection strings, pooling, SSL, migrations, and performance tuning for your NexaFile deployment.
---
...
NexaFile requires **PostgreSQL 14 or later**. No other databases are supported.
...
NexaFile uses two database connection variables:
...
NexaFile automatically detects common database environment variable formats used by hosting providers:
...
If your hosting provider sets these variables, NexaFile will use them automatically.
...
NexaFile uses Prisma, which maintains its own connection pool. Configure pool size with connection string parameters:
...
Database migrations update your schema when upgrading NexaFile.

### Automatic Migrations

When running NexaFile via Docker, migrations run automatically on container startup. No manual intervention is required.
...
Key parameters for NexaFile workloads:
...
NexaFile includes necessary indexes by default. Additional indexes may help for:
