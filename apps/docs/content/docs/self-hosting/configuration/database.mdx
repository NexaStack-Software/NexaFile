...
description: Configure PostgreSQL connection strings, pooling, SSL, migrations, and performance tuning for your NexaSign deployment.
---
...
NexaSign requires **PostgreSQL 14 or later**. No other databases are supported.
...
```
postgresql://nexasign:password@localhost:5432/nexasign
```

**Remote server with SSL:**

```
postgresql://nexasign:password@db.example.com:5432/nexasign?sslmode=require
```
...
```
postgresql://nexasign:p%40ss%23word@localhost:5432/nexasign
```

## Environment Variables

NexaSign uses two database connection variables:
...
```bash
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@host:5432/nexasign
NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://user:password@host:5432/nexasign
```

### Automatic Detection

NexaSign automatically detects common database environment variable formats used by hosting providers:
...
If your hosting provider sets these variables, NexaSign will use them automatically.
...
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@pooler-host:6432/nexasign?pgbouncer=true

# Direct connection (bypasses PgBouncer)
NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://user:password@db-host:5432/nexasign
```
...
NexaSign uses Prisma, which maintains its own connection pool. Configure pool size with connection string parameters:

```
postgresql://user:password@host:5432/nexasign?connection_limit=10&pool_timeout=30
```
...
```
postgresql://user:password@host:5432/nexasign?sslmode=require
```
...
```
postgresql://user:password@host:5432/nexasign?sslmode=verify-full&sslrootcert=/path/to/ca.crt
```
...
docker run -d \
  ...
  -e NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@host:5432/nexasign?sslmode=verify-full&sslrootcert=/etc/ssl/certs/db-ca.crt" \
  NexaStack-Software/NexaSign:latest
```

## Running Migrations

Database migrations update your schema when upgrading NexaSign.

### Automatic Migrations

When running NexaSign via Docker, migrations run automatically on container startup. No manual intervention is required.
...
<Accordions type="multiple">
  ...
  <Accordion title="Connection timeout during migration">
    ...
    ```bash
    NEXT_PRIVATE_DIRECT_DATABASE_URL="postgresql://user:password@host:5432/nexasign?connect_timeout=60"
    ```
  </Accordion>
</Accordions>
...
<Accordions type="multiple">
  ...
  <Accordion title="Neon">
    ...
    **Configuration:**
    ...
    ```bash
    # Pooled connection
    NEXT_PRIVATE_DATABASE_URL=postgresql://[user]:[password]@[endpoint]-pooler.region.aws.neon.tech/nexasign?sslmode=require

    # Direct connection for migrations
    NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://[user]:[password]@[endpoint].region.aws.neon.tech/nexasign?sslmode=require
    ```
  ...
  <Accordion title="AWS RDS">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://nexasign:[password]@your-instance.region.rds.amazonaws.com:5432/nexasign?sslmode=require
    NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://nexasign:[password]@your-instance.region.rds.amazonaws.com:5432/nexasign?sslmode=require
    ```
  ...
  <Accordion title="Google Cloud SQL">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://nexasign:[password]@/nexasign?host=/cloudsql/[project]:[region]:[instance]
    ```
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://nexasign:[password]@[public-ip]:5432/nexasign?sslmode=require
    ```
  ...
  <Accordion title="Azure Database for PostgreSQL">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://nexasign@[server-name]:[password]@[server-name].postgres.database.azure.com:5432/nexasign?sslmode=require
    ```
  ...
  <Accordion title="DigitalOcean Managed Databases">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://doadmin:[password]@[cluster-host]:25060/nexasign?sslmode=require
    ```
...
```bash
pg_dump -h host -U user -d nexasign -F c -f nexasign_backup.dump
```

**Restore from backup:**

```bash
pg_restore -h host -U user -d nexasign -c nexasign_backup.dump
```
...
Key parameters for NexaSign workloads:
...
NexaSign includes necessary indexes by default. Additional indexes may help for:
...
<Accordions type="multiple">
  ...
  <Accordion title="Authentication failed">
    ...
    Solutions:
    - Reset password with `ALTER USER nexasign WITH PASSWORD 'newpassword';`
  ...
  <Accordion title="Database does not exist">
    ...
    ```sql
    CREATE DATABASE nexasign;
    GRANT ALL PRIVILEGES ON DATABASE nexasign TO nexasign;
    ```
  </Accordion>
  <Accordion title="Permission denied">
    ...
    ```sql
    GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO nexasign;
    GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO nexasign;
    ```
  </Accordion>
</Accordions>
