...
NexaFile supports four email transport methods:
...
SMTP is the most common email transport. NexaFile supports two SMTP authentication methods.
...
Generate an API key in the Resend dashboard for NexaFile to use.
...
NexaFile uses React Email templates for all outgoing emails. The templates are located in `packages/email/templates/` and include:
...
- Custom logo replaces the NexaFile logo
...
Configure NexaFile to use the test server:
...
NEXT_PRIVATE_SMTP_FROM_NAME="NexaFile Test"
```
...
```bash
# Docker
docker logs nexasign 2>&1 | grep -i email
...
For MailChannels, configure DKIM directly in NexaFile using the environment variables described above.
