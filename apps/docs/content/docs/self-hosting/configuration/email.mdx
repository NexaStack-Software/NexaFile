...
NexaSign supports four email transport methods:
...
SMTP is the most common email transport. NexaSign supports two SMTP authentication methods.
...
Generate an API key in the Resend dashboard for NexaSign to use.
...
NexaSign uses React Email templates for all outgoing emails. The templates are located in `packages/email/templates/` and include:
...
- Custom logo replaces the NexaSign logo
...
Configure NexaSign to use the test server:
...
NEXT_PRIVATE_SMTP_FROM_NAME="NexaSign Test"
```
...
For MailChannels, configure DKIM directly in NexaSign using the environment variables described above.
