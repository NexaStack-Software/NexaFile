...
description: Configure the X.509 signing certificate used for digital signatures in self-hosted NexaFile.
---

import { Tab, Tabs } from 'fumadocs-ui/components/tabs';

When a document is completed in NexaFile, it is digitally signed using an X.509 certificate. This cryptographic signature provides:
...
Self-hosted NexaFile instances require a signing certificate. You can generate a self-signed certificate or purchase one from a Certificate Authority (CA).
...
For organisations requiring hardware-based key protection, NexaFile supports Google Cloud HSM. This provides:
