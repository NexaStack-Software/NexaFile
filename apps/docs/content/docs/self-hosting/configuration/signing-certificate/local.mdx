...
```bash
docker run -d \
  -v /path/to/certificate.p12:/opt/nexasign/cert.p12:ro \
  -e NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH=/opt/nexasign/cert.p12 \
...
For environments where file mounting is not available:
