...
description: Complete reference for all environment variables used to configure a self-hosted NexaFile instance.
---

## Required Variables

These variables must be set for NexaFile to function:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL of your NexaFile instance (e.g., `https://sign.example.com`)                   |
...
NexaFile requires PostgreSQL 14 or higher.
...
NexaFile supports multiple email transports for sending notifications.
...
NexaFile can store documents in the database or S3-compatible storage.
...
NexaFile requires a certificate to digitally sign documents.
...
NexaFile can use Google Vertex AI for recipient and field detection.
...
NexaFile supports multiple background job providers for processing emails, documents, webhooks, and scheduled tasks.
...
| `NEXT_PRIVATE_REDIS_PREFIX`        | No       | Key prefix for Redis queues (useful when sharing an instance) | `nexasign` |
...
| Variable                      | Description                                  | Default |
| ----------------------------- | -------------------------------------------- | ------- |
| `NEXASIGN_DISABLE_TELEMETRY` | Set to `true` to disable anonymous telemetry | `false` |

Telemetry also auto-disables when `NEXT_PRIVATE_NEXASIGN_LICENSE_KEY` is configured.
...
These variables require an active [Enterprise Edition](/docs/policies/enterprise-edition) license. Obtain a license key from [nexastack.co](https://nexastack.co) and set it below to unlock enterprise features such as SSO, embed authoring, and 21 CFR Part 11 compliance.

| Variable                             | Description                                      |
| ------------------------------------ | ------------------------------------------------ |
| `NEXT_PRIVATE_NEXASIGN_LICENSE_KEY` | License key for enterprise features              |
...
# Database
NEXT_PRIVATE_DATABASE_URL="postgres://user:password@localhost:5432/nexasign"
NEXT_PRIVATE_DIRECT_DATABASE_URL="postgres://user:password@localhost:5432/nexasign"
