...
description: Complete reference for all environment variables used to configure a self-hosted NexaSign instance.
---

## Required Variables

These variables must be set for NexaSign to function:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL of your NexaSign instance (e.g., `https://sign.example.com`)                   |
...
NexaSign requires PostgreSQL 14 or higher.
...
NexaSign supports multiple email transports for sending notifications.
...
NexaSign can store documents in the database or S3-compatible storage.
...
NexaSign requires a certificate to digitally sign documents.
...
NexaSign can use Google Vertex AI for recipient and field detection.
...
NexaSign supports multiple background job providers for processing emails, documents, webhooks, and scheduled tasks.
