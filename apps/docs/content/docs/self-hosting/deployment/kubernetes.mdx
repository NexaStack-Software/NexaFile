---
title: Kubernetes
description: Deploy NexaFile on Kubernetes for production environments requiring high availability, auto-scaling, and enterprise-grade infrastructure.
---
...
A typical NexaFile Kubernetes deployment consists of:
...
Create a dedicated namespace for NexaFile:
...
data:
  ...
  NEXT_PRIVATE_SMTP_FROM_NAME: 'NexaFile'
...
Create the NexaFile Deployment:
...
Update your NexaFile secret to use the in-cluster database:
...
NexaFile stores documents in the database by default. For high-volume deployments, configure S3-compatible storage.
...
Scale NexaFile based on CPU and memory utilization:
