---
title: Kubernetes
description: Deploy NexaSign on Kubernetes for production environments requiring high availability, auto-scaling, and enterprise-grade infrastructure.
---
...
A typical NexaSign Kubernetes deployment consists of:
...
Create a dedicated namespace for NexaSign:
...
metadata:
  name: nexasign
  labels:
    app.kubernetes.io/name: nexasign
```
...
stringData:
  ...
  NEXT_PRIVATE_DATABASE_URL: 'postgresql://user:password@postgres-host:5432/nexasign'
  NEXT_PRIVATE_DIRECT_DATABASE_URL: 'postgresql://user:password@postgres-host:5432/nexasign'
...
data:
  ...
  NEXT_PRIVATE_SMTP_FROM_NAME: 'NexaSign'
  NEXT_PRIVATE_SMTP_FROM_ADDRESS: 'noreply@example.com'
  NEXT_PRIVATE_INTERNAL_WEBAPP_URL: 'http://localhost:3000'
  NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH: '/opt/nexasign/cert.p12'
...
Create the NexaSign Deployment:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: nexasign
          image: NexaStack-Software/NexaSign:latest
          ...
          volumeMounts:
            - name: signing-cert
              mountPath: /opt/nexasign/cert.p12
              subPath: cert.p12
              readOnly: true
      volumes:
        - name: signing-cert
          secret:
            secretName: nexasign-signing-cert
            items:
              - key: cert.p12
                path: cert.p12
```
...
<Callout type="info">
  Pin to a specific image tag (e.g., `NexaStack-Software/NexaSign:1.5.0`) in production instead of `latest`
...
```bash
kubectl create secret generic nexasign-signing-cert \
  --namespace nexasign \
...
type: Opaque
stringData:
  POSTGRES_USER: nexasign
  POSTGRES_PASSWORD: your-secure-password
  POSTGRES_DB: nexasign
---
apiVersion: apps/v1
kind: Deployment
metadata:
...
Update your NexaSign secret to use the in-cluster database:

```yaml
NEXT_PRIVATE_DATABASE_URL: 'postgresql://nexasign:your-secure-password@postgres.nexasign.svc.cluster.local:5432/nexasign'
NEXT_PRIVATE_DIRECT_DATABASE_URL: 'postgresql://nexasign:your-secure-password@postgres.nexasign.svc.cluster.local:5432/nexasign'
```

## Persistent Storage

NexaSign stores documents in the database by default. For high-volume deployments, configure S3-compatible storage.
...
stringData:
  ...
  NEXT_PRIVATE_UPLOAD_BUCKET: 'your-nexasign-bucket'
...
Scale NexaSign based on CPU and memory utilization:
...
<Accordions type="multiple">
  <Accordion title="Pods not starting">
    ...
    ```bash
    # Get pod status
    kubectl get pods -n nexasign
    
    # Describe pod
    kubectl describe pod <pod-name> -n nexasign
    ```
  ...
  <Accordion title="Database connection errors">
    ...
    ```bash
    kubectl run postgres-test --rm -it --image=postgres:15-alpine -n nexasign -- psql "postgresql://user:password@host:5432/database" -c "SELECT 1"
    ```
  </Accordion>
  <Accordion title="Ingress not working">
    ...
    ```bash
    # Get Ingress status
    kubectl get ingress -n nexasign
    
    # Describe Ingress
    kubectl describe ingress nexasign -n nexasign
    ```
  ...
  <Accordion title="Certificate errors">
    ...
    kubectl get secret nexasign-signing-cert -n nexasign -o yaml
    ```
    ...
    kubectl exec -it <pod-name> -n nexasign -- ls -la /opt/nexasign/
    ```
  </Accordion>
