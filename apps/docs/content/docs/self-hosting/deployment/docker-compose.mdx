...
description: Deploy NexaSign with Docker Compose, including PostgreSQL. This production-ready setup is suitable for most self-hosted deployments.
---
...
```bash
mkdir nexasign && cd nexasign
curl -O https://raw.githubusercontent.com/nexastack/nexasign/release/docker/production/compose.yml
```

Alternatively, clone the full repository:

```bash
git clone https://github.com/nexastack/nexasign.git
cd nexasign/docker/production
```
...
services:
  ...
  nexasign:
    ...
    ports:
      - ${PORT:-3000}:${PORT:-3000}
    volumes:
      - /opt/nexasign/cert.p12:/opt/nexasign/cert.p12:ro
...
| `nexasign` | Main application container, waits for database to be healthy |

The NexaSign container waits for the database health check to pass before starting.
...
```bash
# Database (used by both database and nexasign services)
...
# Public URL where NexaSign is accessible
NEXT_PUBLIC_WEBAPP_URL=https://sign.example.com
NEXT_PRIVATE_INTERNAL_WEBAPP_URL=http://localhost:3000

# Database connection (uses Docker service name)
NEXT_PRIVATE_DATABASE_URL=postgresql://nexasign:your-secure-database-password@database:5432/nexasign
...
```bash
sudo mkdir -p /opt/nexasign
sudo cp /path/to/your/cert.p12 /opt/nexasign/cert.p12
sudo chown 1001:1001 /opt/nexasign/cert.p12
sudo chmod 400 /opt/nexasign/cert.p12
```
...
```
NAME                             STATUS              PORTS
nexasign-production-database-1  running (healthy)   5432/tcp
nexasign-production-nexasign-1 running             0.0.0.0:3000->3000/tcp
```
...
```bash
docker compose --env-file .env restart nexasign
```
...
For production, place NexaSign behind a reverse proxy for SSL termination:
...
```bash
# Manual backup
docker compose exec database pg_dump -U nexasign nexasign > backup.sql

# Restore from backup
docker compose exec -T database psql -U nexasign nexasign < backup.sql
```
...
```bash
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@your-db-host:5432/nexasign
NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://user:password@your-db-host:5432/nexasign
```
...
<Accordions type="multiple">
  <Accordion title="Container fails to start">
    Check logs: `docker compose logs nexasign`. Common causes: missing environment variables
  ...
  <Accordion title="Database connection errors">
    ...
    database psql -U nexasign -d nexasign -c "SELECT 1"`.
  </Accordion>
  ...
  <Accordion title="Emails not sending">
    ...
    password, check logs: `docker compose logs nexasign | grep -i smtp`.
  </Accordion>
