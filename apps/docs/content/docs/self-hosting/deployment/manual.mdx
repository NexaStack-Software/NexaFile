...
description: Deploy NexaSign on a Linux server without Docker using Node.js and systemd.
---
...
Navigate to the `nexasign` folder and create a `.env` file:
...
NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@localhost:5432/nexasign"
NEXT_PRIVATE_DIRECT_DATABASE_URL="postgresql://user:password@localhost:5432/nexasign"
NEXT_PRIVATE_SMTP_FROM_NAME="NexaSign"
NEXT_PRIVATE_SMTP_FROM_ADDRESS="noreply@your-domain.com"
```

<Callout type="info">
  If you use a reverse proxy in front of NexaSign, set `NEXT_PUBLIC_WEBAPP_URL` to the public URL
...
Create a service file to run NexaSign as a background service on Linux:

```ini
# /etc/systemd/system/nexasign.service

[Unit]
Description=NexaSign
After=network.target

[Service]
Type=simple
User=www-data
WorkingDirectory=/var/www/nexasign
EnvironmentFile=/var/www/nexasign/.env
...
```bash
sudo systemctl enable nexasign
sudo systemctl start nexasign
```

## Reverse Proxy with Nginx

A minimal Nginx configuration for proxying to NexaSign:
