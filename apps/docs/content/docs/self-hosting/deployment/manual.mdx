...
description: Deploy NexaFile on a Linux server without Docker using Node.js and systemd.
---
...
<Callout type="info">
  If you use a reverse proxy in front of NexaFile, set `NEXT_PUBLIC_WEBAPP_URL` to the public URL
...
Create a service file to run NexaFile as a background service on Linux:

```ini
# /etc/systemd/system/nexasign.service

[Unit]
Description=NexaFile
...
A minimal Nginx configuration for proxying to NexaFile:
