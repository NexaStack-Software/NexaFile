---
title: Railway
description: Deploy NexaSign on Railway with automatic builds from Git, managed PostgreSQL, and built-in networking. Use the one-click template or manual setup.
---
...
<Callout type="warn">
  ...
  [GitHub Discussions](https://github.com/nexastack/nexasign/discussions) for help.
</Callout>
...
Once deployed, Railway provides a URL like `nexasign-production.up.railway.app`.

Click the URL in the Railway dashboard to access your NexaSign instance.
...
If you prefer more control over the setup, deploy NexaSign manually:
...
- Fork or select the [NexaSign repository](https://github.com/nexastack/nexasign)
...
Click on the NexaSign service and go to the "Variables" tab. Add the following variables:
...
| `NEXASIGN_DISABLE_TELEMETRY`     | Disable anonymous telemetry        | `false` |
...
NexaSign runs database migrations automatically on startup.
...
- Click on your NexaSign service
...
Railway provides several options for scaling your NexaSign deployment:
...
<Steps>
<Step>
### Open your NexaSign service

In the Railway dashboard, select the NexaSign service.
...
In the Railway dashboard, select your NexaSign service and open "Settings".
...
<Callout type="warn">
  NexaSign stores sessions in the database, so horizontal scaling works without additional
...
Railway uses usage-based pricing. Estimated monthly costs for NexaSign:
...
NexaSign requires a signing certificate to sign documents. On Railway, you can:

### Option 1: Use Generated Certificate (Default)

NexaSign generates a self-signed certificate automatically if none is provided. This works for testing but shows generic certificate information in signed documents.
...
In your NexaSign service variables, set:
...
[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/nexastack/nexasign)

### Koyeb

<a href="https://app.koyeb.com/deploy?type=git&repository=github.com/nexastack/nexasign&branch=main&name=nexasign-app&builder=dockerfile&dockerfile=/docker/Dockerfile">
  <img
