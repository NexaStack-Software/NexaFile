---
title: Docker
description: Run NexaSign as a standalone Docker container with an external database. Ideal for container platforms like AWS ECS, Google Cloud Run, or Azure Container Instances.
---
...
These variables must be set for NexaSign to start:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL where NexaSign is accessible (e.g., `https://sign.example.com`) | -                             |
...
NexaSign requires two database connection strings:
...
| `NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH`      | Path to signing certificate inside container                   | `/opt/nexasign/cert.p12` |
...
docker run -d \
  ...
  -e NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@db-host:5432/nexasign" \
  ...
  -e NEXT_PRIVATE_SMTP_FROM_NAME="NexaSign" \
...
- **Volume mount** — mount a `.p12` file from the host into the container at `/opt/nexasign/cert.p12` (shown above). This is the simplest approach for small to moderate deployments.
- **Base64-encoded contents** — set `NEXT_PRIVATE_SIGNING_LOCAL_FILE_CONTENTS` with the base64-encoded certificate string. Use this when file mounting is not available.
...
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@db-host:5432/nexasign
...
NexaSign provides health check endpoints for monitoring:
...
```bash
docker inspect --format='{{.State.Health.Status}}' nexasign
```

## Updating the Container

To update NexaSign to a new version:
...
The NexaSign container is stateless. All persistent data is stored in the PostgreSQL database.
...
<Accordions type="multiple">
  <Accordion title="Container fails to start">
    Check the container logs: `docker logs nexasign`. Common causes: missing required environment
