---
title: Docker
description: Run NexaFile as a standalone Docker container with an external database. Ideal for container platforms like AWS ECS, Google Cloud Run, or Azure Container Instances.
---
...
These variables must be set for NexaFile to start:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL where NexaFile is accessible (e.g., `https://sign.example.com`) | -                             |
...
NexaFile requires two database connection strings:
...
docker run -d \
  ...
  -e NEXT_PRIVATE_SMTP_FROM_NAME="NexaFile" \
...
NexaFile provides health check endpoints for monitoring:
...
To update NexaFile to a new version:
...
The NexaFile container is stateless. All persistent data is stored in the PostgreSQL database.
