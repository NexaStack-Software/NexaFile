...
import { tsRestHonoApp } from '@nexasign/api/hono';
import { auth } from '@nexasign/auth/server';
import { jobsClient } from '@nexasign/lib/jobs/client';
import { LicenseClient } from '@nexasign/lib/server-only/license/license-client';
import { createRateLimitMiddleware } from '@nexasign/lib/server-only/rate-limit/rate-limit-middleware';
import {
  aiRateLimit,
  apiTrpcRateLimit,
  apiV1RateLimit,
...
import { TelemetryClient } from '@nexasign/lib/server-only/telemetry/telemetry-client';
import { migrateDeletedAccountServiceAccount } from '@nexasign/lib/server-only/user/service-accounts/deleted-account';
import { migrateLegacyServiceAccount } from '@nexasign/lib/server-only/user/service-accounts/legacy-service-account';
import { env } from '@nexasign/lib/utils/env';
import { logger } from '@nexasign/lib/utils/logger';
import { openApiDocument } from '@nexasign/trpc/server/open-api';
...
// Can be disabled by setting NEXASIGN_DISABLE_TELEMETRY=true
