...
import signingCelebration from '@nexasign/assets/images/signing-celebration.png';
import { getOptionalSession } from '@nexasign/auth/server/lib/utils/get-session';
import { EnvelopeRenderProvider } from '@nexasign/lib/client-only/providers/envelope-render-provider';
import { useOptionalSession } from '@nexasign/lib/client-only/providers/session';
import { AppError, AppErrorCode } from '@nexasign/lib/errors/app-error';
import { getDocumentAndSenderByToken } from '@nexasign/lib/server-only/document/get-document-by-token';
import { viewedDocument } from '@nexasign/lib/server-only/document/viewed-document';
import { getEnvelopeForRecipientSigning } from '@nexasign/lib/server-only/envelope/get-envelope-for-recipient-signing';
import { getEnvelopeRequiredAccessData } from '@nexasign/lib/server-only/envelope/get-envelope-required-access-data';
import { getCompletedFieldsForToken } from '@nexasign/lib/server-only/field/get-completed-fields-for-token';
import { getFieldsForToken } from '@nexasign/lib/server-only/field/get-fields-for-token';
import { getIsRecipientsTurnToSign } from '@nexasign/lib/server-only/recipient/get-is-recipient-turn';
import { getNextPendingRecipient } from '@nexasign/lib/server-only/recipient/get-next-pending-recipient';
import { getRecipientByToken } from '@nexasign/lib/server-only/recipient/get-recipient-by-token';
import { getRecipientSignatures } from '@nexasign/lib/server-only/recipient/get-recipient-signatures';
import { getRecipientsForAssistant } from '@nexasign/lib/server-only/recipient/get-recipients-for-assistant';
import { getTeamSettings } from '@nexasign/lib/server-only/team/get-team-settings';
import { getUserByEmail } from '@nexasign/lib/server-only/user/get-user-by-email';
import { DocumentAccessAuth } from '@nexasign/lib/types/document-auth';
import { extractDocumentAuthMethods } from '@nexasign/lib/utils/document-auth';
import { isRecipientExpired } from '@nexasign/lib/utils/recipients';
import { prisma } from '@nexasign/prisma';
import { SigningCard3D } from '@nexasign/ui/components/signing-card';
    ...
    return (
      <div className="-mx-4 flex max-w-[100vw] flex-col items-center overflow-x-hidden px-4 pt-16 md:-mx-8 md:px-8 lg:pt-16 xl:pt-24">
        ...
        <div className="relative mt-2 flex w-full flex-col items-center">
          ...
          {user ? (
            <Link to="/" className="mt-36 text-primary hover:text-primary/90">
